...
parse.snv = function(sample.dir) {
  ...
  start.dir = getwd()
  print(start.dir)
  new.dir <- file.path(dirname(start.dir), sample.dir)
  setwd(new.dir)
  print(getwd())
  ...
  snv_m2<-isSNV(vcf_m2, singleAltOnly=FALSE) # singleAltOnly=False means consider all mult-alt variants
  ...
  snv_pass=c(rowRanges(vcf_m2[pass_m2&snv_m2]),  #range of values in each row of a matrix
             rowRanges(vcf_f[pass_f&snv_f]),  
             rowRanges(vcf_vs[pass_vs&snv_vs]), 
             rowRanges(vcf_vd[pass_vd&snv_vd]),
             ignore.mcols=T)
  snv_pass= unique(snv_pass) #find all the unique snv_pass cases
  
  #get the respecctive index of each variant caller algo
